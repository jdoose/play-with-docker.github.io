...
If you were running [Docker for Mac](https://docs.docker.com/docker-for-mac/), [Docker for Windows](https://docs.docker.com/docker-for-windows/), or Docker on Linux, you would see your app at http://[YourDockerIP]:32773.
...
You should see your website running [HERE](#){:data-term=".term1"}{:data-port="8888"}
...
To get a new Docker image you can either get it from a registry (such as the Docker Cloud) or create your own. There are hundreds of thousands of images available on [Docker Store](https://Store.docker.com). You can also search for images directly from the command line using `docker search`.
...
- **User images** are images created and shared by users like you. They build on base images and add additional functionality. Typically these are formatted as `user/image-name`. The `user` value in the image name is your Docker Cloud user or organization name.
...
- [Dockerfile](#232-write-a-dockerfile)
...
When you run the `docker image build` command given below, make sure to replace `<YOUR_USERNAME>` with your username. This username should be the same one you created when registering on [Docker Cloud](https://cloud.docker.com). If you haven't done that yet, please go ahead and create an account.
...
If you were running locally on Docker for Mac or Windows, you would head over to `http://localhost:8888` and your app would be live. In this tutorial you can open [your app here](#){:data-term=".term1"}{:data-port="8888"}.
...
{:.quiz}
What command starts a container from the command line?
{:.quiz}
- ( ) docker image build
- ( ) docker container ps
...
{:.quiz}
How do you get a list of running containers?
{:.quiz}
...
{:.quiz}
True or False: Docker only runs Python containers
( ) True
(X) False

{:.quiz}
Which of the following is a Dockerfile command?
- [x] RUN
- [ ] DOCKER
- [x] CMD
- [x] COPY
...
{:.quiz}
What is the first command in every Dockerfile?
- ( ) RUN
- ( ) INDEX
- (x) FROM
